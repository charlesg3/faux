...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        netif_t *waiting[rings];
        int counts[rings];
        int r = netif_select(nifs, waiting, counts, rings, NULL);
        ...
        for(j = 0; j < r; j++)
        {
            netif_packet_t packets[counts[j]];

            int num = netif_recv(waiting[j], packets, counts[j]);
            ...
            for(k = 0; k < num; k++)
            {
                ...
                char *buf = packets[k].data;
                size_t len = *packets[k].len;


                //fprintf(stderr, "Got packet len %lu\n", len);
                ...
                if(ntohs(ether->type) == ETHER_TYPE_ARP)
                {
                        ...
                        ntohs(arp->oper) == ARP_OP_REQUEST)
                    {
                        ...
                        netif_quicksend(&nifs[0], buf, len);
                    }
                }
