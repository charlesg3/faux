...
void server(int id) {
    ...
    Channel * ch;
    do {
        ch = endpoint_accept(server_ep);
    } while (ch == NULL && !shared->quit);
    ...
    while (!shared->quit) {
        ...
        si = (int*)channel_allocate(ch, sizeof(int));
        assert(si);
        *si = rmsg;
        channel_send(ch, si, sizeof(int));
...
main(int _argc, char *_argv[])
{
    ...
    if (argc < 3) {
        printf("%s: Runs many servers for a single client. Servers share an endpoint, created before fork().\n", argv[0]);
    ...
    int total_serving = 0;
    for (int i = 0; i < num_servers; i++) {
        if (shared->served[i] > 0) {
            ++total_serving;
        }
    }
    assert(total_serving == 1);
