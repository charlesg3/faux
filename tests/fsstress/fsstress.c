...
#define	MAXFSIZE	((1ULL << 8) - 1ULL)
#define	MAXFSIZE32	((1ULL << 6) - 1ULL)

static inline __uint64_t __rdtscll()
{
    __uint32_t low_, high_;
    __asm__ __volatile__ ("rdtsc" : "=a" (low_), "=d" (high_));
    return (((__uint64_t) high_ << 32) | (__uint64_t) low_);
}
...
opdesc_t	ops[] = {
	...
	{ OP_TRUNCATE, "truncate", truncate_f, 1, 1 },
	{ OP_UNLINK, "unlink", unlink_f, 1, 1 },
#ifndef NO_XFS
	{ OP_UNRESVSP, "unresvsp", unresvsp_f, 1, 1, 1 },
#endif
	{ OP_WRITE, "write", write_f, 1, 1 },
}, *ops_end;
...
//    uint64_t start_ts = rdtscll();
	...
	ops_end = &ops[nops];
	myprog = argv[0];
	while ((c = getopt(argc, argv, "c:d:e:f:i:n:p:rs:vwzHSX")) != -1) {
		switch (c) {
		case 'd':
			dirname = optarg;
			break;
	...
	if (nproc == 1) { 
		...
		for (i = 0; i < nproc; i++) {
			if (fork() == 0) {
                char id_str[128];
                snprintf(id_str, 128, "-c %d", i);
                new_argv[argc] = id_str;
                new_argv[argc + 1] = 0;
...
append_pathname(pathname_t *name, char *str)
{
	...
	if (len && *str == '/' && name->len == 0) {
		fprintf(stderr, "fsstress: append_pathname failure (%s += %s)\n", name->path, str);
	      ...
	      const int valuelength, int flags)
{
	...
	if (chdir(buf) == 0) {
		rval = attr_set_path(&newname, attrname, attrvalue, valuelength,
			flags);
		__attribute__((unused)) int dummy = chdir("..");
	}
	free_pathname(&newname);
	return rval;
}
...
doproc(void)
{
    ...
    __uint64_t counts[OP_LAST];
    __uint64_t times[OP_LAST];
	...
	for (opno = 0; opno < operations; opno++) {
        int opid = freq_table[random() % freq_table_size];
		p = &ops[opid];
		if ((unsigned long)p->func < 4096) abort(); 

        __uint64_t start = __rdtscll();
        counts[opid]++;
		p->func(opno, random());
        times[opid] += __rdtscll() - start;
		/*
...
/*
    ...
    for(int i = 0; i < OP_LAST; i++)
        if(counts[i])
            fprintf(stderr, "%15s\t%10lld\t%10lld\t%5d%%\n", ops[i].name, counts[i], times[i], times[i] * 100 / total);
...
//	*s++ = '\0';
	...
	for (p = ops; p < ops_end; p++) {
		if (strncmp(arg, p->name, arglen) == 0) {
			p->freq = atoi(s);
			return;
		}
	}
