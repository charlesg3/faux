...
static int
process_tx(int fd, char *ring, const char *pkt, int pktlen, int nframes, int batch)
{
    ...
    D("doing %d frames of %d bytes", nframes, pktlen);
    // fetch a frame
	...
	while (sent >= batch || header->tp_status != TP_STATUS_AVAILABLE) {
	    if (sent) {
		sendto(fd, NULL, 0, 0, (void *) &txring_daddr, sizeof(txring_daddr));
		sent = 0;
		continue;
	    }
	    ...
	    pollset.revents = 0;
	    D("waiting for status avail");
	    ret = poll(&pollset, 1, 1000 /* don't hang */);
	    D("ofs %d poll returns %d status %lx", ring_offset, ret, header->tp_status);
	    if (ret < 0) {
		if (errno != EINTR) {
		    perror("poll");
		    return -1;
...
{
  char *ring, pkt[2048];
  int fd, nframes, framelen, batch;
  // txring device n framelen batch
  if (argc != 5)
	RETURN_ERROR(0, "txring device num-frames framelen batch");

  conf_device = argv[1];
  nframes = atoi(argv[2]);
  framelen = atoi(argv[3]);
  batch = atoi(argv[4]);
  if (framelen < 40 || framelen > sizeof(pkt))
	RETURN_ERROR(0, "wrong framelen");
  ...
  process_tx(fd, ring, pkt, framelen, nframes, batch);
