...
char *version = "$Id: pcap.c 10856 2012-04-06 11:00:57Z luigi $";
int verbose = 0;
...
static int
do_ioctl(struct my_ring *me, int what, __unused int subcmd, __unused int subvalue)
{
	struct ifreq ifr;
	int error;
	int fd = me->fd;
...
pcap_set_promisc(pcap_t *p, int promisc)
{
	...
	D("promisc %d", promisc);
        if (do_ioctl(me, SIOCGIFFLAGS, 0, 0))
	...
	if (do_ioctl(me, SIOCSIFFLAGS, 0, 0))
...
#ifdef __FreeBSD__
        if (do_ioctl(me, SIOCGIFCAP, 0, 0))
		D("SIOCGIFCAP failed");
        me->if_reqcap &= ~(IFCAP_HWCSUM | IFCAP_TSO | IFCAP_TOE);
        if (do_ioctl(me, SIOCSIFCAP, 0, 0))
		D("SIOCSIFCAP failed");
#else /* linux */
                /* disable:
                 ...
                 * - tx-checksumming
                 */
                do_ioctl(me, SIOCETHTOOL, ETHTOOL_SGSO, 0);
                do_ioctl(me, SIOCETHTOOL, ETHTOOL_STSO, 0);
                do_ioctl(me, SIOCETHTOOL, ETHTOOL_SRXCSUM, 0);
                do_ioctl(me, SIOCETHTOOL, ETHTOOL_STXCSUM, 0);
#endif /* !FreeBSD */
