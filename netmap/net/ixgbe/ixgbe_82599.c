                                      ...
                                      u16 soft_id, u8 queue)
{
	...
	case IXGBE_ATR_FLOW_TYPE_SCTPV4:
		if (input_masks->dst_port_mask || input_masks->src_port_mask) {
            ...
            printk(KERN_INFO "flow add error on src/dst port mask\n");
			return IXGBE_ERR_CONFIG;
		}
	case IXGBE_ATR_FLOW_TYPE_TCPV4:
	case IXGBE_ATR_FLOW_TYPE_UDPV4:
	...
	default:
		hw_dbg(hw, " Error on flow type input\n");
            printk(KERN_INFO "error on flow type input\n");
		return IXGBE_ERR_CONFIG;
	}
	...
	default:
        ...
        printk(KERN_INFO "error on VLAN mask\n");
		return IXGBE_ERR_CONFIG;
	}
	...
	if (input_masks->flex_mask & 0xFFFF) {
		if ((input_masks->flex_mask & 0xFFFF) != 0xFFFF) {
            ...
            printk(KERN_INFO "error on flexible byte mask\n");
			return IXGBE_ERR_CONFIG;
		}
...
/**
 *  ixgbe_add_tuple_filter_82599 - Adds a 5-tuple filter
 *  @hw: pointer to hardware structure
 *  @f_prot: Protocol 00b = TCP 1b = UDP 10b = SCTP 11b = Other.
 *  @f_priority: 0-7 filter priority
 *  @input_masks: bitwise masks for relevant fields
 *  @soft_id: software index into the silicon hash tables for filter storage
 *  @queue: queue index to direct traffic to
 *
 ...
 *  hardware writes must be protected from one another.
 **/
s32 ixgbe_add_tuple_filter_82599(struct ixgbe_hw *hw,
                                  ...
                                  uint32_t f_saddr)
{
    u16 port;
        ...
        IXGBE_WRITE_REG(hw, IXGBE_L34T_IMIR(0),
                        (IXGBE_IMIR_SIZE_BP_82599 |
                        IXGBE_IMIR_CTRL_BP_82599));
        IXGBE_WRITE_REG(hw, IXGBE_ETQF(0),
                        (IXGBE_ETQF_FILTER_EN));

        port = swab16(f_dport);
        IXGBE_WRITE_REG(hw, IXGBE_FTQF(0),
                        (IXGBE_FTQF_POOL_MASK_EN |
                        (IXGBE_FTQF_PRIORITY_MASK <<
                        IXGBE_FTQF_PRIORITY_SHIFT) |
                        (IXGBE_FTQF_DEST_PORT_MASK <<
                        IXGBE_FTQF_5TUPLE_MASK_SHIFT)));
        IXGBE_WRITE_REG(hw, IXGBE_SDPQF(0), (port << 16));
        ...
        IXGBE_WRITE_REG(hw, IXGBE_SYNQF, 0x80000100);
