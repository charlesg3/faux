...
int num_rings_override = 0;
module_param( num_rings_override, int, 0);

int RSS_enable = 1;
module_param(RSS_enable, int, 0);
    ...
    if (adapter->flags & IXGBE_FLAG_FDIR_HASH_CAPABLE) {
        for (i = 0; i < adapter->num_tx_queues; i++)
            adapter->tx_ring[i]->atr_sample_rate =
                adapter->atr_sample_rate;
        //printk(KERN_INFO "SIGNATURE IXGBE\n");
        ixgbe_init_fdir_signature_82599(hw, adapter->fdir_pballoc);
    } else if (adapter->flags & IXGBE_FLAG_FDIR_PERFECT_CAPABLE) {
        //printk(KERN_INFO "PERFECT IXGBE\n");
...
static int ixgbe_set_num_queues(struct ixgbe_adapter *adapter)
{
    ...
    if (num_rings_override != 0)
    {
        /* command line override for numnber of rings */
        adapter->num_rx_queues = num_rings_override;
        adapter->num_tx_queues = num_rings_override;
    } 
...
static int __devinit ixgbe_sw_init(struct ixgbe_adapter *adapter)
{
    ...
    if (RSS_enable) adapter->flags |= IXGBE_FLAG_RSS_ENABLED;
